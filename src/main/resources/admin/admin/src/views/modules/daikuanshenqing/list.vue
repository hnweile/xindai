<template>
	<div class="main-content" :style='{"color":"#666","padding":"30px","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","flexWrap":"wrap","justifyContent":"space-between","display":"flex"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"margin":"10px 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('daikuanshenqing','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 0px","fontSize":"inherit","color":"inherit","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('daikuanshenqing','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"0px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="抵押资产">
						<template slot-scope="scope">
							{{scope.row.diyazichan}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' prop="zichancailiao" label="资产材料">
						<template slot-scope="scope">
							<el-button v-if="scope.row.zichancailiao" type="text" size="small" @click="download(scope.row.zichancailiao)">下载</el-button>
							<span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('daikuanshenqing','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('daikuanshenqing','信用评估')" @click="xinyongpingguCrossAddOrUpdateHandler(scope.row,'cross','','','[1]','请勿重复操作!')" type="success">
								<span class="icon iconfont icon-qita3" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","height":"40px"}'></span>
								信用评估
							</el-button>
							<el-button class="edit" v-if=" isAuth('daikuanshenqing','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('daikuanshenqing','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<xinyongpinggu-cross-add-or-update v-if="xinyongpingguCrossAddOrUpdateFlag" :parent="this" ref="xinyongpingguCrossaddOrUpdate"></xinyongpinggu-cross-add-or-update>
...
<script>
	...
	import xinyongpingguCrossAddOrUpdate from "../xinyongpinggu/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				xinyongpingguCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			xinyongpingguCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.xinyongpingguCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','daikuanshenqing');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.xinyongpingguCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.xinyongpingguCrossaddOrUpdate.init(row.id,type);
				});
			},
