		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","flexWrap":"wrap","justifyContent":"space-between","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"margin":"10px 0","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">专员账号</label>
						<el-input v-model="searchForm.zhuanyuanzhanghao" placeholder="专员账号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">专员姓名</label>
						<el-input v-model="searchForm.zhuanyuanxingming" placeholder="专员姓名" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"margin":"10px 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('fengkongzhuanyuan','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 0px","fontSize":"inherit","color":"inherit","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('fengkongzhuanyuan','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"0px"}'>
				<el-table class="tables"
					...
					:border='true'
					v-if="isAuth('fengkongzhuanyuan','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('fengkongzhuanyuan','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('fengkongzhuanyuan','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('fengkongzhuanyuan','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		methods: {
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.zhuanyuanzhanghao!='' && this.searchForm.zhuanyuanzhanghao!=undefined){
					params['zhuanyuanzhanghao'] = '%' + this.searchForm.zhuanyuanzhanghao + '%'
				}
 				if(this.searchForm.zhuanyuanxingming!='' && this.searchForm.zhuanyuanxingming!=undefined){
					params['zhuanyuanxingming'] = '%' + this.searchForm.zhuanyuanxingming + '%'
				}
