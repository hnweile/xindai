		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","flexWrap":"wrap","justifyContent":"space-between","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"margin":"10px 0","display":"block"}' >
					...
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}' class="select" label="评估等级" prop="pinggudengji">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">评估等级</label>
						<el-select clearable v-model="searchForm.pinggudengji" placeholder="请选择评估等级" >
							<el-option v-for="(item,index) in pinggudengjiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}' class="select" label="评估结果" prop="pinggujieguo">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">评估结果</label>
						<el-select clearable v-model="searchForm.pinggujieguo" placeholder="请选择评估结果" >
							<el-option v-for="(item,index) in pinggujieguoOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"margin":"10px 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('xinyongpinggu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 0px","fontSize":"inherit","color":"inherit","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('xinyongpinggu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"0px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="zhuanyuanxingming"
						label="专员姓名">
						<template slot-scope="scope">
							{{scope.row.zhuanyuanxingming}}
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('xinyongpinggu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('xinyongpinggu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('xinyongpinggu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				...
				if(this.searchForm.pinggudengji!='' && this.searchForm.pinggudengji!=undefined){
					params['pinggudengji'] = this.searchForm.pinggudengji
				}
				if(this.searchForm.pinggujieguo!='' && this.searchForm.pinggujieguo!=undefined){
					params['pinggujieguo'] = this.searchForm.pinggujieguo
				}
