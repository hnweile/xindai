<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="保证人" prop="baozhengren">
				<el-input v-model="ruleForm.baozhengren" 
					placeholder="保证人" clearable :disabled=" false  ||ro.baozhengren"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="抵押资产" prop="diyazichan">
				<el-input-number v-model="ruleForm.diyazichan" placeholder="抵押资产" :disabled=" false ||ro.diyazichan"></el-input-number>
			</el-form-item>
			<el-form-item class="add-item" label="资产材料" prop="zichancailiao">
				<file-upload
					tip="点击上传资产材料"
					action="file/upload"
					:limit="1"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`daikuanshenqing/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
					...
					if(valid) {
						if(this.type=='cross'){
							...
							if(crossrefid && crossuserid) {
								...
								await this.$http.get('daikuanshenqing/list', {
									params: params
								}).then(res => {
						...
						await this.$http.post(`daikuanshenqing/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
			...
			zichancailiaoUploadChange(fileUrls) {
				this.ruleForm.zichancailiao = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
		}
	};
</script>
