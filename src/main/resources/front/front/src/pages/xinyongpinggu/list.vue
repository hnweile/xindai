	...
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('xinyongpinggu','新增')" type="primary" @click="add('/index/xinyongpingguAdd')">
					添加
				</el-button>
			</el-form>
			<div class="select2">
						...
						<div class="infoBox">
							...
							<div class="centerInfo">
								<div class="publisher_item">
									<span class="icon iconfont" :class="index%2==0?'icon-touxiang18':'icon-touxiang18'"></span>
									<span class="label">{{index%2==0?'发布人：':'发布人：'}}</span>
									<span class="text">{{item.zhuanyuanzhanghao}}</span>
								</div>
							</div>
...
<script>
	export default {
		...
		async created() {
			...
			this.selectOptionsList.push({name:'评估等级',list:this.pinggudengjiOptions,tableName: 'pinggudengji',check: -1})
			this.pinggujieguoOptions = '通过,不通过'.split(',');
			this.selectOptionsList.push({name:'评估结果',list:this.pinggujieguoOptions,tableName: 'pinggujieguo',check: -1})
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.pinggudengji != '') searchWhere.pinggudengji = this.formSearch.pinggudengji;
				if (this.formSearch.pinggujieguo != '') searchWhere.pinggujieguo = this.formSearch.pinggujieguo;
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`xinyongpinggu/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/xinyongpingguDetail', query: params});
			},
